<template>
    <article class="open-title-input" :class="addingGroup">
        <span v-if="!isEditingTitle" @click="isEditingTitle = true" class="add-entity-placeholder flex">
        ...
        <div v-else class="flex flex-column">
            ...
            <div class="flex align-center">
                <button @click="addEntity" class="add-entity-btn">{{ btnPlaceholder }}</button>
                <button @click="closeAddEntity" class="close-add-entity"><svg stroke="currentColor" fill="currentColor"
...
export default {
    ...
    methods: {
        addEntity() {
            switch (this.groupOrTask) {
                case 'group': this.addGroup()
                    break
                case 'task': this.addTask()
                    break
            }
        },
        addGroup() {
        ...
        addTask() {
            ...
            console.log(newTask);
            this.$store.dispatch({ type: 'saveTask', groupId: this.groupId, task: newTask, activity: {} })
            this.titleInput = ''
        },
        closeAddEntity() {
            this.titleInput = ''
    ...
    computed: {
        ...
        openInputPlaceholder() {
            switch (this.groupOrTask) {
                case 'group': return 'Add another list'
                case 'task': return 'Add a card'
            }
        },
        inputPlaceholder() {
            switch (this.groupOrTask) {
                case 'group': return 'Enter list title...'
                case 'task': return 'Enter a title for this card...'
            }
        },
        btnPlaceholder() {
            switch (this.groupOrTask) {
                case 'group': return 'Add list'
                case 'task': return 'Add card'
            }
        }
    },
}
