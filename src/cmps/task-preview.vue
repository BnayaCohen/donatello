<template>
  ...
  <div v-show="isOpen" class="quick-card-editor">
    <span class="icon-lg icon-close quick-card-editor-close-icon trellicons trellicons-close-btn">
    </span>
    <div :style="getCords" v-click-outside="closeQuickEdit" class="quick-card-editor-card">
      <div :style="task?.styles" class="list-card list-card-quick-edit is-covered">
        <div class="list-card-details">
          <div v-if="task?.labels?.length" class="list-card-labels">
            <span v-for="label in task?.labels" :key="label.id" :style="`backgroundColor:${label.color}`">{{ label
            }}</span>
          </div>
          <textarea data-autosize="true" dir="auto" style="
              overflow: hidden;
          ...
          <div class="badges">
            <span class="badges-container">
              ...
              <div v-if="task?.comments?.length" class="badge comments">
                <i class="fa-regular fa-message"></i><span>{{ task?.comments.length }}</span>
              </div>
              <div v-if="task?.attachment?.length" class="badge attachments">
                <i class="fa-regular fa-link"></i><span>{{ task?.attachment.length }}</span>
              </div>
            </span>
          </div>
          <div v-if="task?.members?.length" class="list-card-members">
            <div v-for="member in task.members" :key="member._id" class="member"
              :style="`backgroundImage:url(${member.imgUrl}),border-radius:50%,height:30px,width:30px`"></div>
          </div>
        </div>
      </div>
      ...
      <div class="quick-card-editor-buttons fade-in">
        ...
        <span href="#" class="quick-card-editor-buttons-item"><span class="icon-sm icon-label"><i
        ...
        <span href="#" class="quick-card-editor-buttons-item"><span class="icon-sm icon-card-cover"><i
...
export default {
  ...
  computed: {
    ...
    getChecklistProgress() {
      const checklistCounter = this.task.checklists.reduce((acc, checklist) => {
